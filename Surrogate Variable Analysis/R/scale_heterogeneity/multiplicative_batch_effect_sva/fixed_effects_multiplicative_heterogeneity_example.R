############################################################################################################
## fixed_effects_multiplicative_heterogeneity_example.R
## There are two classes and each class is a Gaussian mixture. The Gaussian mixture will be consider as het
## -erogeneity. Especially in this case, the value SVA will be applied to alliviate this problem. SVA is expected
...
# Multiplicative Batch effect: For Batch 1, b1 = 0.5; For Batch 2, b2 = 2.
# Random Noise: N(0, 1)
...
if(t2 > t1){
  for(i in (t1 + 1) : t2){
    M[,i] = rep(c(2, -2), c(40, 40)) + 
      c(rnorm(n_11, 0, 1), rnorm(n_12, 0, 2), rnorm(n_21, 0, 1), rnorm(n_22, -2, 2)) 
  }
}

if(t3 > t2){
  for(i in (t2 + 1) : t3){
    M[,i] =c(rnorm(n_11, 0, 1), rnorm(n_12, 0, 2), rnorm(n_21, 0, 1), rnorm(n_22, -2, 2))
  }
}
...
save(train0, train1.1, train1.2, train2.1, train2.2, 
     train3.1, train3.2, train4, fe, file = "rdata/scale_hetero_example.RData")
