...
# Angle between batch label vector and class label vector
...
g2 = ggplot(data = Cor_sv_df, aes(x = pi_1, y = pi_2, fill = acos(cor_sv_bv) /pi * 180)) + 
  ...
  labs(x = expression(pi[1]), y = expression(pi[2]), fill = 'Angle',
       title = "Angles Between Surrogate Variable From Modified SVA and Batch Label Vector") +
  scale_x_discrete(breaks = seq(from = 1, to = 41, by = 10),labels = c(0:4)/4) +
  scale_y_discrete(breaks = seq(from = 1, to = 41, by = 10),labels = c(0:4)/4) +
  geom_text(x = 21, y = 21, label = "X", col = "red", size = 2) +
  geom_text(x = 1, y = 1, label ="O", col = "black", size = 2) +
...
g4 <- ggplot(data = Cor_sv_df, aes(x = pi_1, y = pi_2, fill = acos(cor_sv_pc1) /pi * 180)) + 
  ...
  scale_fill_gradient2(breaks =c(0:9)*10, limits=c(0, 90), high = "white", mid = "blue") +
  labs(x = expression(pi[1]), y = expression(pi[2]), fill = 'angle',
       title = "Angles Between Surrogate Variable From Modified SVA and PC1") +
  scale_x_discrete(breaks = seq(from = 1, to = 41, by = 10),labels = c(0:4)/4) +
  scale_y_discrete(breaks = seq(from = 1, to = 41, by = 10),labels = c(0:4)/4) +
  geom_text(x = 21, y = 21, label = "X", col = "red", size = 2) +
  geom_text(x = 1, y = 1, label ="O", col = "black", size = 2) +
...
pdf(file = 'figures/Geometric_Analysis_of_Modified_SVA/angle/angle_y_bv.pdf')
print(g1)
dev.off()

pdf(file = 'figures/Geometric_Analysis_of_Modified_SVA/angle_new_sv_bv.pdf')
print(g2)
dev.off()

pdf(file = 'figures/Geometric_Analysis_of_Modified_SVA/angle_new_bv_pc1.pdf')
print(g3)
dev.off()

pdf(file = 'figures/Geometric_Analysis_of_Modified_SVA/angle_new_sv_pc1.pdf')
print(g4)
dev.off()
