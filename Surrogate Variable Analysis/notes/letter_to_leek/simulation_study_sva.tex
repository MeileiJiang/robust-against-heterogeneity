...
As shown in Figure~\ref{fig:svas1}\subref{fig:sva1}, the input data matrix in Case 1 is the contamination of the matrices in Figure~\ref{fig:heatmap}\subref{fig:type_c} and Figure~\ref{fig:heatmap}\subref{fig:type_d}. Figure~\ref{fig:svas1}\subref{fig:sva1} shows that the data matrix after removing batch effects through IRW-SVA has the same pattern as the original simulation data set, which indicate that IRW-SVA fails to adjust batch effects in this case. Figure~\ref{fig:svas1}\subref{fig:new_sva1} shows that after removing the batch effects through modified, the rows of Type C genes, which affected by both class and batch effects, have been recovered as the same signal in Figure~\ref{fig:heatmap}\subref{fig:type_a}. This indicates modified SVA adjusts batch effects and recovers the pattern of measured factor quite well in Case 1. 

In Figure~\ref{fig:visual1}, the x-axis and y-axis represent gene index and probability respectively, and the point of each gene is colored by its type. Figure~\ref{fig:visual1}\subref{fig:pprob1_2} shows the posterior probability of each gene affected by class effects and we expect that Type C genes have high probability while Type D genes have low probabilty. Figure~\ref{fig:visual1}\subref{fig:pprob1_2} indicates that both IRW-SVA and modified SVA identify the genes affected by class effects.
Figure~\ref{fig:visual1}\subref{fig:pprob1_1} shows the posterior probability of each gene affected by batch effects. The left panel of Figure~\ref{fig:visual1}\subref{fig:pprob1_1} shows that IRW-SVA can not correctly identify the genes associated with batch label, while the right panel of Figure~\ref{fig:visual1}\subref{fig:pprob1_1} indicates modified SVA is able to correctly identify the genes associated with batch label. This is not surprised since IRW-SVA relies on Type B Genes to recover the signal of batch effects while the data set in Case 1 does not contain Type B genes.
